...
export default class TrackCommand implements BaseDiscordCommand {
  data = new SlashCommandBuilder()
    .setName("track")
    .setDescription(
      "Allows you to track top plays and replays. Also enable weekly updates of your profile."
    )
  ...
  async run(interaction: CommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (!member.permissions.has("ADMINISTRATOR") && !guild.admin_channel) {
        const embed = new MessageEmbed()
          .setDescription(
      ...
      const trackChannel = interaction.guild.channels.cache.get(
        guild.track_channel
      );
      ...
      console.error(error);
      const embed = new MessageEmbed().setDescription(error.message);
