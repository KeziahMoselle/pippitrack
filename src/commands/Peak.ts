...
export default class PeakCommand implements BaseDiscordCommand {
  data = new SlashCommandBuilder()
    .setName("peak")
    .setDescription(
      "Display peak rank, accuracy, playcount, replay watched and recent rank."
    )
  ...
  async run(interaction: CommandInteraction): Promise<void> {
    ...
    try {
      const response = await axios.get(
        `https://osutrack-api.ameo.dev/peak?user=${user.id}&mode=${getModeInt(
          mode
        )}`
      );
      ...
      const bestPlaycount = user.monthly_playcounts.reduce(function (
        prev,
        current
      ) {
      ...
      const bestReplayWatched = user.replays_watched_counts.reduce(function (
        prev,
        current
      ) {
        return prev.count > current.count ? prev : current;
      ...
      const embed = new MessageEmbed()
        ...
        .addField(
          "Most playcount in a month",
          `${intl.format(new Date(bestPlaycount.start_date))} with ${
            bestPlaycount.count
          } playcount.`
        )
        .addField(
          "Best replay watched",
          `${intl.format(new Date(bestReplayWatched.start_date))} with ${
            bestReplayWatched.count
          } replay${bestReplayWatched.count > 0 ? "s" : ""} watched.`
        )
        .setFooter({
        ...
        .setURL(
          `https://ameobea.me/osutrack/user/${encodeURIComponent(
            user.username
          )}`
        )
        .setColor(11279474);
